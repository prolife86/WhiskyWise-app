...
### ⚠️ Notes
- Android may show a warning about installing unknown apps — this is expected behaviour.
...
└── src/main/java/com/whiskywise/app/
    ...
    └── ui/
        ...
        ├── statistics/               # Stats summary, breakdown bars
        └── settings/                 # Sessions + tokens with revoke, logout
```
...
| `GET` | `/api/v1/stats` | Dashboard summary + currency symbol *(currency requires server ≥ v1.6.6)* |
| `GET` | `/api/barcode-lookup` | Barcode lookup *(requires server ≥ v1.6.0)* |
...
GitHub Release published (e.g. v0.3.2)
        ...
        ▼
extract-version job
  strips 'v' prefix → "0.3.2"
  converts to versionCode → 302  (major×10000 + minor×100 + patch)
        │
        ▼
build job
